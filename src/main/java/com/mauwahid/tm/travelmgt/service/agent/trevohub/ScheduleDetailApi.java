...
public class ScheduleDetailApi extends TrevoHubRequester  {

    private FlightSearchDetailReq flightSearchDetailReq;
    ...
    private static final Logger logger = LoggerFactory.getLogger(SearchScheduleApi.class);


    public void setReqSearch(FlightSearchDetailReq flightSearchDetailReq){
        this.flightSearchDetailReq = flightSearchDetailReq;
    }


    public ResSearchDetail populateDetailSchedules(){

        ResSearchDetail resSearchDetail;

        params = getMapParams();
        params.put("session_id_depart", flightSearchDetailReq.getSessionIdDepart());
        params.put("schedule_key_depart", flightSearchDetailReq.getScheduleKeyDepart());
        params.put("schedule_class_depart", flightSearchDetailReq.getScheduleClassDepart());
        params.put("session_id_return", flightSearchDetailReq.getSessionIdReturn());
        params.put("schedule_key_return", flightSearchDetailReq.getScheduleKeyReturn());
        params.put("schedule_class_return", flightSearchDetailReq.getScheduleClassReturn());

        String jsonResponse = "";

        if(flightSearchDetailReq.getAirlineType().equalsIgnoreCase("domestic")){
            try{

                jsonResponse = reqDomesticAirlines();

                resSearchDetail = translateFromJson(jsonResponse);


            }catch (Exception ex){
                logger.error("Error Exception "+ex.toString());
                resSearchDetail  = new ResSearchDetail();
                resSearchDetail.setErrorNo("");
              //  resSearchDetail.(StatusCode.S_API_ERROR_CALL);
            }
            ...
            try{
                ...
                logger.error("Error Exception "+ex.toString());
                resSearchDetail = new ResSearchDetail();
              //  resSearchDetail.setApiId(apiId);
              //  resSearchDetail.setStatus(StatusCode.S_API_ERROR_CALL);
            }
        }
